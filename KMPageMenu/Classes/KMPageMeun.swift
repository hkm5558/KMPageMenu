...
public struct KMPageMenuStyle {
    ...
    /// 指示器风格
    public var indicatorStyle: KMPageMenuIndicatorStyle = .cover(widthType:.sizeToFit(minWidth: 10))
    
    /// 指示器圆角
    public var indicatorCorner: KMPageMenuIndicatorCornerStyle = .semicircle
    
    /// label 宽度类型 固定宽度 或 随文字适应宽度
    public var labelWidthType: KMPageMenuItemWidthType = .sizeToFit(minWidth: 50)
    
    /// 指示器 宽度类型 固定宽度 或 随文字适应宽度
    public var indicatorWidthType: KMPageMenuItemWidthType {
...
@IBDesignable public class KMPageMenu: UIControl {
    ...
    public convenience init(frame: CGRect, titles: [String]) {
        self.init(frame: frame, segmentStyle: KMPageMenuStyle(), titles:  titles)
    }
    
    public init(frame: CGRect, segmentStyle: KMPageMenuStyle, titles: [String]) {
    ...
    private func reloadData() {
        ...
        let tapGesture = UITapGestureRecognizer(target: self, action: #selector(KMPageMenu.handleTapGesture(_:)))
...
fileprivate extension KMPageMenu {
